/**
 * @file TargetRegistry.hpp
 * @brief Singleton registry for managing Lynx platform bitcode libraries.
 * 
 * This class maintains the mapping of bitcode libraries for various combinations of
 * Lynx target types, operating systems, and CPU architectures. It provides utilities
 ...
 */
 ...
 namespace LynxPlatform {
     ...
     class TargetRegistry {
         ...
         // Dispatch libraries
         const Bitcode *m_dispatch;        ///< Dispatch module for general platforms
         const Bitcode *m_dispatch_macos;  ///< Dispatch module specifically for macOS
 
         // Builtins-c libraries keyed by OS x ArchType
         std::map<uint32_t, const Bitcode *> m_builtins; ///< Builtins-c bitcode libraries
         std::map<uint32_t, const Bitcode *> m_targets;  ///< ISPC target libraries
         std::map<uint32_t, const Bitcode *> m_stdlibs;  ///< Stdlib bitcode libraries
 
         std::bitset<(int)TargetOSType::error> m_supported_oses; ///< Bitset of supported OSes
     ...
     public:
         ...
         /**
          * @brief Returns the target-specific library for a given target, OS, and architecture.
          * @param target Lynx target type.
          * @param os Target operating system.
          * @param arch Target architecture.
          * @return Pointer to the target Bitcode if available, nullptr otherwise.
          */
