...
namespace LynxAst {
    llvm::Value* TernaryExpressionNode::generateCode(std::shared_ptr<AstContext> astContext) {
        ...
        llvm::Value* condition = this->conditionExpression->generateCode(astContext->createContext());

        // Create two basic blocks (true and false branches)
        llvm::BasicBlock* trueBB = llvm::BasicBlock::Create(context, lynxTernaryTrueBB, function);
        llvm::BasicBlock* falseBB = llvm::BasicBlock::Create(context, lynxTernaryFalseBB, function);
        llvm::BasicBlock* mergeBB = llvm::BasicBlock::Create(context, lynxTernaryMergeBB, function);  // Merge block for PHI node
        ...
        llvm::Value* trueExp = this->thenExpression->generateCode(astContext->createContext());
        ...
        llvm::Value* falseExp = this->endExpression->generateCode(astContext->createContext());
    ...
    std::unique_ptr<Node> TernaryExpressionNode::clone() const {
        return std::make_unique<TernaryExpressionNode>(
            ...
            this->thenExpression->clone(),
            this->endExpression->clone()
        ); 
    }
